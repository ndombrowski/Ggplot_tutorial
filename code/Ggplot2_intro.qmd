...
1. Initialize the plot using the `ggpplot()`function in the first line of code. Inside the function, we provide the input dataframe (i.e. penguins_clean) and we also  
...
3. Define how we want to plot our data in the second line of code. For example, we specify that we want to generate a scatterplot via ``geom_point()``. When using the `+` symbol after the first ggplot call, we add a **layer** to our plot. We can add as many layers to our plot as we want, more on that later.
...
![](../images/saving_images.png){width=100% fig-align="left"}
...
3. Make a scatterplot to compare bill length and bill depth. Map the shape aesthethic to the island variable. Also change the size attribute to make the symbols bigger and make the dots more transparent (it is up to you what values you choose).
...
If you are unsure what is happening in the code when we use the `labs` function, run the code without `labs(x = "Species")`. We will explain how to customize labels and other parts of the plot in more detail in @sec-change-legend. 
...
There are more ways that you can use the forcats library to order data, but that's out of the scope of this tutorial. For more, feel free to start by having a look at the [forcats documentation](https://forcats.tidyverse.org/reference/index.html).
...
For violin points, its a bit harder to get the individual points inside the violin plot if you are interested in making this work we can recommend looking at the [ggbeeswarm](https://github.com/eclarke/ggbeeswarm) library.
...
Below we use simple words, i.e. red and blue, to describe what colors we want to use. However, its recommended to use a [hexcode](https://www.w3schools.com/colors/colors_hexadecimal.asp) instead to have a bit more control over our color scheme. For example #377EB8 would also give a blue color (and in newer RStudio versions, the hexcode gets highlighted by the color the ID represents).

Let us start by defining some vectors in which we define the colors we want to map our three species too:

1. palette1 in which we are explicit about what species is mapped to what color
2. palette2 in which we only create a vector with three written out colors
3. palette3 in which we create a vector with colors using a hexcode
...
2. Depending on how you assign the colors, you might not have control over the order of the colors and need to watch out that the colors get assigned to the right variable, i.e. species in the examples above. 
...
`ggstatsplot` is an extension of `ggplot2` package for creating graphics with details from statistical tests included in the information-rich plots themselves. More information can be found [here](https://indrajeetpatil.github.io/ggstatsplot/).
