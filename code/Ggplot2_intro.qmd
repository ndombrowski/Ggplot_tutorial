...
If you don't have the palmerpenguins data set and the tidyverse downloaded then you can type the following into your R notebook or console:
...
If you are opening the notebook version (i.e. the qmd file) of this tutorial, you will sometimes see a little comment before the actual code, i.e. `#| eval: false`. This comment simply controls how the code chunk behaves when rendering the document to a HTML and can be ignored when following the tutorial.
...
- The beaver dataset, a time series recording the temperature and activity of two beavers. Time-series are best represented in line plots and we will explore this data when talking about these types of plots.

Let's have a first look at the penguin dataset and let us first have a look at the first few rows:
...
Next, let's explore the data structure (`str` let's us know if your columns are factors, integers or numbers):
...
Finally, let's calculate a basic summary of our data:

```{r}
summary(penguins)
```

After our first data exploration steps, we can see that we have eight columns with different variables, such as species, bill length or year. For each data column we got a summary of the number of observations (for factor data) as well as basic statistics for all numerical data. Another useful piece of information we get is the number of missing values for each column. 

Since we can see that the penguin dataset has rows with missing data, our first task is to remove any row with missing data:

:::{.callout-important}

If you are working in base R you might have an error when using `|>` if you are working an R version below 4.1. If you get an error and can't update or don't want to update R use `%>%` instead. For this to work, you need to have the tidyverse library loaded.
...
Now we can start to generate our first plot. Let's start with a scatterplot. In the code below you see the following:

1. In the first line of the code we initialize the plot using the `ggpplot`function. We provide the input dataframe (i.e. penguins_clean) and we also  
...
3. In the second line of code, we define how we want to plot our data, i.e. in the example below we want to generate a scatterplot via ``geom_point()``. When using the `+` symbol after the first ggplot call, we add a layer to our plot. We can add as many layers to our plot as we want, more on that later.
...
Plot the body mass on the y-axis and the species on the x-axis and generate a scatterplot.
...
In this example we generate a dot instead of a scatter plot.  
This plot is not ideal since points are on top of each other but we later learn how we could improve this.
...
Plots can be saved as variables and we can add more layers to these variables using the `+` operator. This is really useful if you want to make multiple related plots from a common base, i.e. we could store a common theme in one variable and reuse it for different plot types. Below we start:
...
We can use different data frames on each layer. To generate an example, lets fit a model on our data (without going into the details) and also plot it on top of our points:
...
Now, we can add our predictions as a line to our first scatter plot by adding an additional layer in which we provide our second dataset:
...
Make a scatterplot comparing the flipper and bill lengths and assign the islands to different colors. It is up to you where you assign the color aesthethic and feel free to try both options.
...
1. Make a scatterplot to compare bill length and bill depth. Change the color for the islands. Also change the size attribute to make the symbols bigger and make the dots more transparent (it is up to you what values you choose).
2. Make a scatterplot to compare bill length and bill depth and use different colors for different islands. In the point layer, change the shape attribute to 21.
3. Do the same as in exercise 2 but instead of mapping the islands to the colors map them to the fill aesthetic instead. Do you understand what is happening?
...
Comment on using fill versus color:  
...
Now, lets plot the observations. You will notice that compared to the first time we used `geom_bar()` we added `stat = "identity"` to our geom. 

A statistical transformation, or stat, transforms the data, typically by summarizing it in some manner. Geom_bar by default uses `stat = "count"` to automatically count the number of observations without us having to write this out. If we instead want to provide already existing values to geom_bar instead of letting it automatically count for us, we have to change this argument to `stat = "identity"`:
...
Alternatively, we could use `geom_col`. `geom_col()` won't try to aggregate the data by default and it expects us to already have the y values calculated and uses these values directly.  
...
We can facet with one group in vertical direction by adding another layer with `facet_grid()`:
...
We can also facet using two variables:
...
#this way facet_grid won't separate the points by species for this datafram
df2 <- penguins_clean |> 
  select(-species)
...
1. Generate a scatterplot comparing the bill length and body mass. Add a trendline (color the trendline in grey) and use facet_grid to create subplots for different species. Its up to you whether you prefer a horizontal or vertical orientation.
...
4. Do the same as in exercise 3 but use facet_wrap instead. Do you see how the behave differently?
