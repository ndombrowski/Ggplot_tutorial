...
data.tb <- tibble(x = 0, y = 6400, tb = list(summary))
...
We can also insert plots with `geom_plot_npc()`

```{r}
#generate a boxplot and store it in a variable
boxplot <-
ggplot(penguins_clean, aes(species, bill_depth_mm )) +
  geom_boxplot() +
...
ggplot(penguins_clean, aes(bill_length_mm, bill_depth_mm, color = species)) + 
  geom_point() + 
  theme_manual +
  annotate("plot_npc", npcx = "left", npcy = "bottom", label = boxplot) +
  ylim(0,24) + 
  xlim(0,62)
...
ggplot(penguins_clean, aes(bill_length_mm, bill_depth_mm)) +
  ...
  annotate(geom = "rect", 
           xmin = 49, xmax = 61, ymin = 13, ymax = 21,
           linetype = "dotted", fill = NA, colour = "black") +
  geom_point() +
  annotate("segment",  x = 49, xend = 59.5, y = 21, yend = 29.5, linetype = "dotted") +
  annotate("segment",  x = 61, xend = 79, y = 13, yend = 23, linetype = "dotted") +
  theme_manual
``` 
