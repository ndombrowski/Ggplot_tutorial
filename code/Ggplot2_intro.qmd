...
Visualizing data is important not only for publishing but also to explore our data. For example, imagine that we do a regression analysis and we get the result two variables (i.e. weight and income) are positively correlated. While this might be an exciting result, without visualizing the data we might miss issues with outliers or don't see that are data points are not distributed normally. That is why plotting the data is so important. For example in the plot below, we have the exact same regression line but the data points behave very differently.
...
Since we can see that the penguin dataset has rows with missing data (i.e. NA), our first task is to remove any row with missing data:
...
In this way, we can easily change how the plot looks, i.e. instead of a dot plot, we can have a boxplot (more about boxplots later):
...
The  `plot()` function takes as argument file name and if needed the directory we want to save our file in. We can add additional arguments such as the width or height of our plot or even the target paper size. 
...
We can plot different data frames on each layer. To generate an example, lets fit a model on our data and also plot it on top of our points. While going into how to fit models is out of the scope of this tutorial, free free to look at the links below if you want to go deeper:
